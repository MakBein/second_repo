...
#  API Endpoints
# ============================================================
def attack_api_endpoints(session: requests.Session, base_url: str, endpoints: List[str], headers_list: List[Dict[str, str]], log: Optional[LogFunc] = None) -> List[Dict[str, Any]]:
        ...
        for hdr in headers_list:
            try:
                ...
                res = {
                    ...
                    "severity": _classify_severity(r.status_code),
                }
                results.append(res)
            except Exception as e:
                _safe_log(log, f"[API] ❌ {url}: {e}", "warn")
                results.append({"endpoint": url, "error": str(e), "severity": "error"})
    return results

# ============================================================
#  Token Brute Force
# ============================================================
def brute_force_tokens(session: requests.Session, base_url: str, tokens: List[str], log: Optional[LogFunc] = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for token in tokens:
        ...
        try:
            ...
            results.append({"token": token, "error": str(e), "severity": "error"})
    return results

# ============================================================
#  Parameters
# ============================================================
def attack_parameters(session: requests.Session, base_url: str, parameters: List[str], log: Optional[LogFunc] = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for param in parameters:
        ...
        try:
            ...
            res = {
                ...
                "reflected": reflected,
                "length": len(body),
                "severity": _classify_severity(r.status_code, reflected=reflected),
            }
            results.append(res)
        except Exception as e:
            _safe_log(log, f"[PARAM] ❌ {base_url} param={param}: {e}", "warn")
            results.append({"parameter": param, "error": str(e), "severity": "error"})
    return results

# ============================================================
#  User IDs
# ============================================================
def attack_user_ids(session: requests.Session, base_url: str, user_ids: List[Any], log: Optional[LogFunc] = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for uid in user_ids:
        ...
        try:
            ...
            r = session.get(url, timeout=10, verify=False)
            body = r.text or ""
            leaked_email = bool(re.search(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", body))
            res = {
                "user_id": uid,
                "url": url,
                "status": r.status_code,
            ...
            results.append({"user_id": uid, "error": str(e), "severity": "error"})
    return results

# ============================================================
#  XSS Targets
# ============================================================
def attack_xss_targets(session: requests.Session, base_url: str, targets: List[Any], log: Optional[LogFunc] = None) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    for t in targets:
        ...
        try:
            ...
            dom_suspicious = any(k in body.lower() for k in ("innerhtml", "document.write", "location.hash", "eval(", "settimeout("))
            res = {
                "url": full_url,
                "base_url": url,
                "payload": payload,
            ...
            _safe_log(log, f"[XSS] ❌ {full_url}: {e}", "warn")
            results.append({"url": full_url, "payload": payload, "context": context, "error": str(e), "severity": "error"})
    return results
