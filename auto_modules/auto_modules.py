...
def _safe_get(
    ...
    """
    Безопасный GET: никогда не бросает исключения наружу.
...
def attack_api_endpoints(
    ...
    for ep in endpoints:
        ...
        for hdr in headers_list:
            ...
            r = _safe_get(session, url, headers=hdr)
            if r is None:
                results.append({
                    "endpoint": url,
                    "headers_used": hdr,
...
def brute_force_tokens(
    ...
    for token in tokens:
        ...
        r = _safe_get(session, base_url, headers=headers)
        if r is None:
            results.append({
                "token": token,
                "error": "request_failed",
