...
class ThreatSenderMixin:
    """
    ThreatSenderMixin — универсальный миксин для отправки артефактов
    в ThreatConnector/Threat Intel.

    • Формирует унифицированный payload
    • Поддерживает dict, list и простые значения
    • Логирует ошибки через self.log (если доступен)
    """
    ...
    def send_to_threat_intel(
        ...
        """
        Отправляет данные в ThreatConnector.

        :param module_name: имя модуля-источника
        :param data: данные (dict, list или простое значение)
        """
